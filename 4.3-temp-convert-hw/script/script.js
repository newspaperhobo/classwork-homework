...
function convertFarenheitToCelcius(tempF) {
    let tempC = Math.round((tempF - 32) * (5 / 9))
    return tempC;
}

if (isNaN(tempF)) {
    document.getElementById("tempC").innerHTML = `That's not a temperature! You didn't enter a number, please refresh the page!`
} else {
    let tempC = convertFarenheitToCelcius(tempF);
    console.log(tempC);
